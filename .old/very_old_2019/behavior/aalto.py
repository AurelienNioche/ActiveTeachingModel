...
class UserTask(Task):
    ...
    def __init__(self, user_id,
        ...
        super().__init__(n_iteration=self.question_entries[-1].t,
...
class UserData(Data):
    ...
    def __init__(self, user_id, normalize_similarity=False, verbose=False):
        ...
        n_iteration = self.tk.n_iteration

        questions = np.zeros(n_iteration, dtype=int)
        replies = np.zeros(n_iteration, dtype=int)
        possible_replies = np.zeros((n_iteration, self.tk.n_possible_replies),
                                         dtype=int)
        self.success = np.zeros(n_iteration, dtype=int)
