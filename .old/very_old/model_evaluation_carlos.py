...
class SimulationAndFit:
    ...
    def __init__(self, model, n_iteration=300, n_kanji=30, grade=1,
                 normalize_similarity=False,
                 verbose=False, **kwargs):

        self.model = model

        self.tk = Task(n_iteration=n_iteration, n_kanji=n_kanji, grade=grade,
...
def main(model, max_=20, n_iteration=300, n_kanji=30, normalize_similarity=True,
         force=False, **kwargs):

    extension = f'_{model.__name__}{model.version}_n{max_}_t{n_iteration}_k{n_kanji}'
    ...
    if not os.path.exists(file_path) or force:
        ...
        results = list(tqdm(pool.imap_unordered(
            SimulationAndFit(model=model, n_iteration=n_iteration, n_kanji=n_kanji,
                             normalize_similarity=normalize_similarity,
                             **kwargs), seeds
        ), total=max_))
...
if __name__ == "__main__":
    ...
    # main(ActR, max_=100, n_kanji=79, n_iteration=100, force=True)
    main(Exponential, max_=100, n_kanji=79, n_iteration=1000, force=True)
