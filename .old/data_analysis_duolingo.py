...
def count(selection, verbose=True):
    ...
    for s in selection:
        ...
        _n_iteration = np.sum(s_seen)
        _n_item = len(np.unique(lexeme_id))

        n_iteration.append(_n_iteration)
        n_item.append(_n_item)

    if verbose:

        best_idx = np.argsort(n_iteration)[-1]
        print("*** USER WITH THE MOST ITERATIONS ***")
        print('user_id:', selection[best_idx])
        print('n_iteration:', n_iteration[best_idx])
        print('n_item:', n_item[best_idx])

    return n_iteration, n_item


def n_trial(u_id='u:iOIr'):
    """
    :param u_id: User id (string)
    :return: n_iteration and number of unique lexeme (tuple)
    """
...
def info_user(u):
    ...
    print('n iterations:', u.n_iteration)
...
def main(model=ActR):
    ...
    for user_id in selection:
        ...
        if u.learning_language is not None and u.n_iteration > 1000:
            info_user(u)
            min_u = min(u.times)
            max_u = max(u.times)
            sec = max_u - min_u
            ...
            raw_times = np.zeros(u.n_iteration)
            raw_times[:] = u.times
            ...
            agent = model(
                tk=Task(n_iteration=u.n_iteration, n_item=u.n_item),
                # param={'d': 0.5, 'tau': 0.01, 's': 0.3, 'r': 0.001})
                param=fit_r["best_param"])

            for i in range(u.n_iteration):
    ...
    # n_iteration, n_item = count(s)
    # best_idx = np.argsort(n_iteration)[-2]
