...
def run(student_model, teacher_model,
        student_param=None, n_item=25, grades=(1, ), n_iteration=250):
    ...
    assert teacher_model in \
        (Active, RandomTeacher, LeitnerTeacher, TraditionalLeitnerTeacher), \
        "Teacher model not recognized."

    teacher = teacher_model(n_iteration=n_iteration, n_item=n_item, grades=grades)
    ...
    print(f"\nSimulating data with a student {student_model.__name__} "
          ...
          f"using {n_item} kanji of grade {grades} for {n_iteration} time steps...",
          end=" ", flush=True)
...
def main():
    ...
    for teacher_model in (TraditionalLeitnerTeacher, Active,
                          LeitnerTeacher, RandomTeacher):
        run(student_model=ActRMeaning, teacher_model=teacher_model, n_iteration=550)
