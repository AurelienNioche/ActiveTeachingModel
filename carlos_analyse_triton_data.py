...
def main():
    ...
    trial_name = "fake-files"  # input("Trial name: ")
    ...
    for timing, timing_label in items_learnt.items():

        fig_path = os.path.join(fig_folder, f"{trial_name}_chocolate_{timing}.pdf")
        chocolate.plot(
            df=df,
            teachers=teachers,
            learners=learners,
            psychologists=psy,
            fig_path=fig_path,
            learnt_label=timing_label,
        )

        # fig_path = os.path.join(fig_folder, f"{trial_name}_box_{timing}.pdf")
        # box.plot(df=df, fig_path=fig_path, learnt_label=timing_label)

        # fig_path = os.path.join(fig_folder, f"{trial_name}_hist_{timing}.pdf")
        # hist.plot(learnt_label=timing_label, df=df, fig_path=fig_path)
