...
class TaskParam:
    ...
    def __init__(self, is_item_specific, time_between_ss,
                 time_per_iter, n_ss, ss_n_iter, learnt_threshold):
...
class Config:
    ...
    def __init__(self,
                 ...
                 agent,
                 bounds,
                 md_learner, md_psy, md_teacher,
                 omni, n_item,
                 task_pr_lab, task_pr_val,
                 teacher_pr_lab, teacher_pr_val,
                 psy_pr_lab, psy_pr_val,
                 pr_lab, pr_val):
        ...
        self.task_pr = TaskParam(**{k: v for k, v in
                                    zip(task_pr_lab, task_pr_val)})
        self.psy_pr = {k: v for k, v in zip(psy_pr_lab, psy_pr_val)}
        self.teacher_pr = {k: v for k, v in zip(teacher_pr_lab,
                                                teacher_pr_val)}
    ...
    def get(cls, file):
        ...
        with open(file) as f:
            ...
            cf = cls(config_file=os.path.basename(file), config_dic=cf_from_file,
                     **cf_from_file)
        return cf
